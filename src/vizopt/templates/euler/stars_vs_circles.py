...
class EulerDiagram(VizOptimizer):
    ...
    def _build_problem(self) -> OptimizationProblem:
        ...
        pos_scale_x = max(
            float(np.std(circles_array[:, 0])), float(circle_radii.mean())
        )
        pos_scale_y = max(
            float(np.std(circles_array[:, 1])), float(circle_radii.mean())
        )
        ...
        terms = [
            ...
            ObjectiveTerm(
                "area", wrap(_multi_term_area), self.weight_area, schedules.get("area")
            ),
            ObjectiveTerm(
                "perimeter",
    ...
    def sets_(self) -> list[dict]:
        ...
        radii_arr = np.array(
            self.representation.to_radii(optim_vars, jnp.array(angles))
        )
    ...
    def circles_(self) -> np.ndarray:
        ...
        return np.concatenate(
            [
                np.array(self.result_.optim_vars["circle_positions"]),
                circle_radii[:, None],
            ],
            axis=1,
        )
...
def _make_plot_configuration(
    ...
    def plot_configuration(
        ...
        for s, (name, color) in enumerate(zip(set_names, colors)):
            ...
            radii = radii_arr[s]
            bx = np.append(
                cx + radii * np.cos(angles), cx + radii[0] * np.cos(angles[0])
            )
            by = np.append(
